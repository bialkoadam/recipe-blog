...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold mb-6">{{$recipe->name}} szerkesztése</h1>
    ...
    <form action="{{ route('recipes.update', $recipe->id) }}" method="POST" enctype="multipart/form-data" class="max-w-lg mx-auto">
        @csrf
        @method('PUT')
        <div class="mb-6">
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Recept név</label>
            <input type="text" name="name" id="name" value="{{old('name', $recipe->name)}}" class="block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
        </div>
        <div class="mb-6">
        ...
        <div class="mb-6">
            ...
            <select name="difficulty_id" id="difficulty_id" class="block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                ...
                <option value="{{ $difficulty->id }}" {{ (old('difficulty_id', $recipe->difficulty_id) == $difficulty->id) ? 'selected' : '' }}>{{ $difficulty->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-6">
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" class="block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" rows="4">{{old('description', $recipe->description)}}</textarea>
        </div>
        <div class="mb-6">
            <label for="ingredients" class="block mb-2 text-sm font-medium text-gray-900">Hozzávalók</label>
            <textarea name="ingredients" id="ingredients" class="block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" rows="4">{{old('ingredients', $recipe->ingredients)}}</textarea>
        </div>
        <div class="mb-6">
            <label for="instruction" class="block mb-2 text-sm font-medium text-gray-900">Elkészítés</label>
            <textarea name="instruction" id="instruction" class="block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" rows="4">{{old('instruction', $recipe->instruction)}}</textarea>
        </div>
        <div class="mb-6">
            <label for="time" class="block mb-2 text-sm font-medium text-gray-900">Sütési/főzési idő (perc)</label>
            <input type="number" name="time" id="time" value="{{old('time', $recipe->time)}}" class="block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
